...
class MetalTexture: NSObject {
  ...
  func loadTexture(device device: MTLDevice, commandQ: MTLCommandQueue, flip: Bool){
    ...
    var context = CGBitmapContextCreate(nil, width, height, bitsPerComponent, rowBytes, colorSpace, CGImageAlphaInfo.PremultipliedLast.rawValue)
  ...
  class func textusource(source source:MTLTexture,device: MTLDevice, mipmaped: Bool) -> MTLTexture {
  ...
  func image(mipLevel mipLevel: Int) -> UIImage{
    ...
    var context = CGBitmapContextCreate(p, mipmapedWidth, mipmapedHeight, 8, rowBytes, colorSpace, CGImageAlphaInfo.PremultipliedLast.rawValue)
